...
async function api<T>(path: string, headers: Record<string, string>, init?: RequestInit): Promise<T> {
  ...
  // NestJS returns an empty 200 when this optional medical record does not exist.
  if (response.status === 200 && (init?.method ?? 'GET') === 'GET' && /^\/medical\/clearances\/[^/]+$/.test(path)) {
