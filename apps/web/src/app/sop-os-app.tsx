...
function LeadModal({ connected, onClose, onCreated }: { connected: boolean; onClose: () => void; onCreated: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try { await api('/leads', { method: 'POST', body: JSON.stringify({ dataProvenance: 'synthetic', code: data.get('code'), firstName: data.get('firstName'), lastName: data.get('lastName'), email: data.get('email'), phone: data.get('phone'), sourceType: data.get('sourceType'), campusId }) }); onCreated(); }
