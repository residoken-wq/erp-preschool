...
| Audit/Outbox | platform + worker | audit_events, outbox_events, outbox_delivery_attempts, security_events | sửa/xóa audit; domain gửi provider trực tiếp |
| Documents | Phase 1 adapter | application_documents metadata; object binary ngoài DB | tải pending/infected; public object key |
| Traceability/Rules/Approval | Phase 1 mới | migration `0008`/`0009` dự kiến | JSON không schema/owner để né model |
...
|---|---|---|---|
| Tenant FK không composite | nhiều FK chỉ reference UUID, dù row có `organization_id` | cross-org reference | `0007`, composite candidate key/FK + negative test |
...
| Trace BR/FR/AC/Test chưa có schema | chỉ tài liệu baseline | mất lineage | `0008_governance_traceability.sql` |
| Approval/rule config chưa có | approved_by fields rời rạc | SoD/policy inconsistency | P1-E06/07, `0008`/`0009` |
...
| Secure file path chưa hoàn chỉnh | metadata có storage key/scan status, chưa adapter/access grant | HRI malware/leak | P1-E09, `0010` |
| Admission relationship/consent/checklist còn thiếu | person/application baseline tối giản | legal/privacy/flow blocker | refine SOP-ADM-001; `0011` |
| Provider thật và reconciliation UI chưa có | typed runtime hiện chỉ cho development console sink ngoài production | không thể delivery production | DEC-015; thêm approved adapter + manual reconciliation, giữ fail-closed production guard |
