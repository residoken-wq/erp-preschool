...
3. Thêm reviewer khác biệt, gán CODEOWNERS theo domain và bật required approval/
   CODEOWNERS review cho `main`; bootstrap protection đã được áp dụng.
...
| Repository protection bootstrap | PARTIAL PASS: strict `quality`/`compose-config`, PR flow, admin enforcement, linear history, conversation resolution; force-push/delete off; required review chờ collaborator thứ hai |
