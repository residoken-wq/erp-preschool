...
Migration `0001`–`0006` đã tồn tại và được xem là immutable sau khi apply. `0006`
lưu delivery-attempt metadata cho outbox; không lưu payload, thuộc scope
organization, data class tối thiểu `CON`, không tự xóa khi retention chưa được duyệt.
...
| `0007_platform_scope_hardening.sql` | org/campus scope, composite tenant constraints, row version/idempotency base | ngăn cross-org FK; test empty + upgrade DB | forward-fix; backup trước DDL rủi ro |
| `0008_governance_traceability.sql` | BR/FR/AC/Test/TraceLink và approval primitives | version/link history không bị overwrite | additive trước, backfill có reconciliation |
| `0009_rule_configuration.sql` | scoped/versioned/effective rule + approval/audit | unique effective version theo scope/time strategy | disable version; không delete evidence |
| `0010_secure_documents.sql` | metadata/quarantine/scan/access/audit, không lưu binary DB | state/tenant constraint, random object key | revoke access + reconcile orphan object |
| `0011_admission_extensions.sql` | missing Lead-to-Handover fields/tables theo slices | state prerequisites, tenant FK, money decimal | expand/migrate/contract; reconcile counts |
